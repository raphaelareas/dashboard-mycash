-- ============================================
-- MIGRATION 017: Corrigir despesas fixas no cartão para contar apenas uma vez por mês
-- Despesas fixas devem contar apenas a transação do mês atual/passado mais recente
-- ============================================
      ...
      ELSIF account_type_val = 'CREDIT_CARD' THEN
        ...
        IF OLD.type = 'EXPENSE' THEN
          ...
          IF is_fixed_expense THEN
            -- Para despesas fixas, só conta se for a transação mais recente do mês atual/passado
            -- Verificar se existe outra transação da mesma série com data mais recente ou igual
            SELECT NOT EXISTS (
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.account_id IS NOT NULL THEN
      ...
      ELSIF account_type_val = 'CREDIT_CARD' AND OLD.type = 'EXPENSE' THEN
        ...
        IF is_fixed_expense THEN
          -- Verificar se era a transação mais recente
          SELECT NOT EXISTS (
            SELECT 1 FROM transactions t
            WHERE t.account_id = OLD.account_id
            AND t.description = OLD.description
    ...
    IF NEW.account_id IS NOT NULL THEN
      ...
      ELSIF account_type_val = 'CREDIT_CARD' AND NEW.type = 'EXPENSE' THEN
        ...
        IF is_fixed_expense THEN
          -- Para despesas fixas, só conta se for a transação mais recente do mês atual/passado
          -- Verificar se não existe outra transação da mesma série com data mais recente ou igual
          SELECT NOT EXISTS (
            SELECT 1 FROM transactions t
